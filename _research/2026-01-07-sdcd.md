...
3.  **Contrastive Calibration:** We subtract the logits of the disrupted view from the original view:
    $$ P_{SDCD}(y|x) \propto \text{softmax}( (1+\alpha) \cdot \text{logit}(y|V, x) - \alpha \cdot \text{logit}(y|V', x) ) $$

where $\alpha \geq 0$ is a contrastive hyperparameter controlling the penalty strength on texture bias.
